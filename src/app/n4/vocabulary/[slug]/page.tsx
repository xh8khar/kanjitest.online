...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      url: `https://kanjitest.online/n4/vocabulary/${slug}`,
    },
    twitter: { title: `${vocab.word} (${vocab.reading}) — JLPT N4 Vocabulary`, description: `${vocab.word} (${vocab.reading}) — ${vocab.english}.` },
    alternates: { canonical: `https://kanjitest.online/n4/vocabulary/${slug}` },
  }
}

export default async function N4VocabDetail({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params
  const decoded = decodeURIComponent(slug)
  const vocab = getVocabulary("n4").find((v) => v.slug === decoded)
  if (!vocab) notFound()

  const idx = getVocabulary("n4").findIndex((v) => v.slug === decoded)
  const prev = idx > 0 ? getVocabulary("n4")[idx - 1] : null
