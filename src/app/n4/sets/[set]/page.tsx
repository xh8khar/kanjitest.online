import TestClient from "./TestClient"
import type { Metadata } from "next"
import { siteUrl, keywords as kw } from "@/lib/seo"

export function generateStaticParams() {
  return Array.from({ length: 20 }, (_, i) => ({ set: String(i + 1) }))
}

export async function generateMetadata({ params }: { params: Promise<{ set: string }> }): Promise<Metadata> {
  const { set } = await params
  const desc = `Practice JLPT N4 kanji with Set ${set} of 20 questions. Test your kanji-to-reading and reading-to-kanji skills.`
  return {
    title: `Kanji Test Online – JLPT N4 Kanji Quiz Set ${set} (20 Questions)`,
    description: desc,
    keywords: kw([`kanji test online set ${set}`, `JLPT N4 kanji test set ${set}`, `kanji quiz online set ${set}`, `N4 kanji quiz set ${set}`]),
    openGraph: {
      title: `Test Set ${set} — JLPT N4 Kanji Quiz`,
      description: desc,
      url: siteUrl(`/n4/sets/${set}/`),
    },
    twitter: { title: `Test Set ${set} — JLPT N4 Kanji Quiz`, description: desc },
    alternates: { canonical: siteUrl(`/n4/sets/${set}/`) },
  }
}

export default function TestSetPage({ params }: { params: Promise<{ set: string }> }) {
  return <TestClient params={params} />
}
