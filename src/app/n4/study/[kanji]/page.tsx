  ...
  return params.then(async ({ kanji }) => {
    ...
    return {
      ...
      openGraph: {
        title: `${k.kanji} (${k.meanings[0]}) — JLPT N4 Kanji`,
        description: desc,
        url: `https://www.kanjitest.online/n4/study/${k.kanji}/`,
      },
      twitter: { title: `${k.kanji} (${k.meanings[0]}) — JLPT N4 Kanji`, description: desc },
      alternates: { canonical: `https://www.kanjitest.online/n4/study/${k.kanji}/` },
    }
  })
}
...
export default async function KanjiDetail({ params }: { params: Promise<{ kanji: string }> }) {
  ...
  const { prev, next } = getPrevNext(k.id, "n4")
  const setNum = getSetForKanji(k.id, "n4")
  ...
  const breadSchema = breadcrumbSchema([
    ...
    { name: k.kanji, url: `/n4/study/${k.kanji}/` },
  ])
  ...
  return (
    <div className="px-4 py-6 sm:py-8">
      ...
      <nav className="text-[11px] sm:text-xs text-ink/50 mb-3 sm:mb-4 flex flex-wrap items-center gap-0.5">
        ...
        <span>/</span>
        <a href="/n4/" className="text-ink/50 hover:text-ink transition-colors px-1.5 sm:px-2 py-1.5">N4</a>
        <span>/</span>
        <a href="/n4/study/" className="text-ink/50 hover:text-ink transition-colors px-1.5 sm:px-2 py-1.5">Study</a>
        <span>/</span>
      ...
      <div className="flex flex-wrap gap-2 mb-5 sm:mb-6">
        <Link href="/n4/flashcards/" className="border border-ink/20 rounded-lg h-10 sm:h-11 px-4 sm:px-5 text-xs sm:text-sm text-ink/70 hover:border-ink/40 hover:text-ink transition-all flex items-center gap-1">
          🃏 Flashcards
        </Link>
        <Link href={`/n4/sets/${setNum}/`} className="border border-ink/20 rounded-lg h-10 sm:h-11 px-4 sm:px-5 text-xs sm:text-sm text-ink/70 hover:border-ink/40 hover:text-ink transition-all flex items-center gap-1">
          ✍️ Test
        </Link>
      </div>
      ...
      <div className="flex gap-2">
        {prev ? (
          <Link href={`/n4/study/${prev.kanji}/`} className="flex-1 border border-ink/20 rounded-lg h-12 flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 text-xs sm:text-sm text-ink/70 hover:border-ink/40 hover:text-ink transition-all">
            <span>&larr;</span>
        ...
        {next ? (
          <Link href={`/n4/study/${next.kanji}/`} className="flex-1 border border-ink/20 rounded-lg h-12 flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 justify-end text-xs sm:text-sm text-ink/70 hover:border-ink/40 hover:text-ink transition-all">
