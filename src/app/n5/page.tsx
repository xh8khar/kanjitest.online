...
export const metadata: Metadata = {
  ...
  description:
    "Browse all 79 JLPT N5 kanji with kun/on readings, example words, and daily-life sentences. Study, flashcard, and test yourself.",
...
export default function N5Hub() {
  const all = getAll()
  const items = all.map((k) => ({ name: `${k.kanji} — ${k.meanings[0]}`, url: `/n5/study/${k.kanji}/` }))
  ...
  return (
    <div className="px-4 py-6 sm:py-8">
      ...
      <h1 className="text-lg sm:text-xl font-bold text-ink">All 79 Kanji</h1>
      <p className="text-xs sm:text-sm text-ink/60 mt-1 mb-6 sm:mb-8">JLPT N5 — the most fundamental Japanese characters</p>
      ...
      <div className="flex gap-1.5 sm:grid sm:grid-cols-3 md:grid-cols-4 overflow-x-auto pb-1 sm:pb-0 sm:gap-3 mb-6 sm:mb-10 scrollbar-none">
        ...
        >
          ...
          <p className="hidden sm:block text-xs text-ink/70 mt-0.5">79 characters</p>
        </Link>
        <Link
          href="/n5/flashcards/"
          className="shrink-0 sm:shrink border border-ink/20 rounded-xl px-3 sm:px-5 py-2.5 sm:py-5 bg-white hover:border-ink/40 transition-all sm:text-center flex sm:block items-center gap-2"
        ...
        >
          ...
          <p className="hidden sm:block text-xs text-ink/70 mt-0.5">219 words</p>
        </Link>
        <Link
          href="/n5/sets/"
          className="shrink-0 sm:shrink border border-ink/20 rounded-xl px-3 sm:px-5 py-2.5 sm:py-5 bg-white hover:border-ink/40 transition-all sm:text-center flex sm:block items-center gap-2"
