  ...
  return (
    <div className="px-4 py-6 sm:py-8">
      ...
      <div className="flex items-center justify-between mt-5 sm:mt-6">
        {prev ? (
          <Link href={`/n5/vocabulary/${prev.slug}/`} className="text-xs sm:text-sm text-ink/60 hover:text-ink transition-colors">&larr; {prev.word}</Link>
        ) : <div />}
        {next ? (
          <Link href={`/n5/vocabulary/${next.slug}/`} className="text-xs sm:text-sm text-ink/60 hover:text-ink transition-colors">{next.word} &rarr;</Link>
        ) : <div />}
      </div>
    </div>
  )
