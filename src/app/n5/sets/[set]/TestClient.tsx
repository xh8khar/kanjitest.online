...
export default function TestClient({
  ...
  if (isNaN(setNum) || setNum < 1 || setNum > 20) {
    return <div className="px-4 py-8 text-center">
      ...
      <Link href="/n5/sets/" className="text-ink/70 hover:text-ink transition-colors text-sm mt-4 inline-block">&larr; All sets</Link>
    </div>
  }
...
function TestContent({ setNum }: { setNum: number }) {
  const questions = getSetQuestions(setNum)
  ...
  return (
    <div className="px-4 py-8">
      ...
      {/* Score summary — shows only when all answered */}
      {allDone && (
