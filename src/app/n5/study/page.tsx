...
export const metadata: Metadata = {
  title: "JLPT N5 Kanji — Complete Study List with Readings",
  description:
    "Complete list of 79 JLPT N5 kanji with kun/on readings, example words, and daily sentences. Click any kanji to start learning.",
  ...
  openGraph: {
    ...
    url: "https://www.kanjitest.online/n5/study",
  },
  twitter: { title: "JLPT N5 Kanji — Complete Study List with Readings", description: "79 JLPT N5 kanji with readings, examples, and sentences." },
  alternates: { canonical: "https://www.kanjitest.online/n5/study" },
}

export default function N5StudyList() {
  const all = getAll()
  const items = all.map((k) => ({ name: `${k.kanji} (${k.kun[0]?.replace(/\..+$/, "") || k.on[0]}) — ${k.meanings[0]}`, url: `/n5/study/${k.kanji}/` }))
  const listSchema = itemListSchema(items)
  const pageSchema = collectionPageSchema("JLPT N5 Kanji — Complete Study List with Readings", "79 JLPT N5 kanji with readings, examples, and sentences.", "/n5/study")
