            ...
            <div key={lv.prefix}>
              ...
              {lv.exists ? (
                <div className="flex flex-col gap-0.5 mt-1.5">
                  <Link href={`${lv.prefix}/study/`} className="text-sm text-ink/60 hover:text-ink transition-colors">Study</Link>
                  <Link href={`${lv.prefix}/flashcards/`} className="text-sm text-ink/60 hover:text-ink transition-colors">Flashcards</Link>
                  <Link href={`${lv.prefix}/vocabulary/`} className="text-sm text-ink/60 hover:text-ink transition-colors">Vocabulary</Link>
                  <Link href={`${lv.prefix}/sets/`} className="text-sm text-ink/60 hover:text-ink transition-colors">Test</Link>
                </div>
              ) : (
                <p className="text-sm text-ink/40 mt-1.5">Coming soon</p>
              )}
            </div>
          ))}
        </div>

        <div className="flex flex-wrap items-center justify-center gap-x-5 gap-y-1 text-sm mt-8 pt-5 border-t border-ink/10">
